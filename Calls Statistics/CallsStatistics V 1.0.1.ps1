...
function GeneraterichText([string]$text, [int]$x, [int]$y, [int]$width, [int]$height){
    ...
    $richTextBox.Font = New-Object System.Drawing.Font("Times New Roman",10,[System.Drawing.FontStyle]::Regular)
    return $richTextBox
}
#-------------------------------function--------------------------------------------------------#
function Clean(){
...
function SaveButton{
    if($MainTopicBox.SelectedValue -ne "אנא בחר נושא מהרשימה"){
        if($MainTopicBox.SelectedItem -eq "אחר"){
           $CsvRow = new-object PSObject -property @{name = $env:USERNAME
        ...
        elseif($SubtopicBox.SelectedValue -eq "אחר"){
            ...
            $CsvRow|Export-Csv "D:\script\zahi\test\Calls Statistics\CallsStatistics.csv" -NoTypeInformation -Encoding UTF8 -Append
            [void][System.Windows.Forms.MessageBox]::Show("The operation successfully completed :)","Calls Statistics") 
        }
        Clean
    }
...
'כספות' = 'איפוס סיסמאות','תהליך לא עובד','יצירת כספת','יצירת תהליך';
'Microsof Office' = 'Outlook','Word','Excel','Powerpoint','OneDrive','התקנת office';
'ניהול דוא"ל' = 'ניהול רשימות תפוצה','הגדלת תיבה','הגדלת כמות נמענים','ארכיון','הגדרת מייל בפלאפון','הוספת לו"ז למשתמש';
'Sccm' = 'התקנת/תיקון Client';
'Active Directory' = 'Disabled user','Security groups','Modify attributes';
'תפעול מחשב'='כונן רשת';
}
...
#-------------------------------topic-------------------------------------#
$MainTopicBox = GenComboBox -data ($ListSubject.Keys+="אחר") -x '140' -y '20' -width '100'
$MainTopicBox.DropDownWidth=($MainTopicBox.DataSource|%{$_.Length}|Sort-Object -Descending|select -First 1) *7
...
$MainTopicBox.RightToLeft ="yes"
    $MainTopicBox.add_SelectedValueChanged{
        ...
        elseif($MainTopicBox.SelectedValue -ne "אנא בחר נושא מהרשימה"){
                ...
                $Data.AddRange([array]$ListSubject[$MainTopicBox.SelectedValue])
