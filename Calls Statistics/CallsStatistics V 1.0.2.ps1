...
#-------------------------------Object--------------------------------------------------------#
$JSONFile= get-Content "\\docserver1\SYSTEM_DOCS\files\PowerShell\Subjects.json"|ConvertFrom-Json
$ListSubject = @{}
$JSONFile | get-member -MemberType NoteProperty | `
    Where-Object{ -not [string]::IsNullOrEmpty($JSONFile."$($_.name)")} | `
    ForEach-Object {$ListSubject.add($_.name,$JSONFile."$($_.name)")}
